...
-spec be_ok({ok, GZG} | {error, any()}) -> GZG.
be_ok(A) ->
    case A of
        {ok, Value} ->
            Value;
...
-spec be_error({ok, any()} | {error, GZL}) -> GZL.
be_error(A) ->
    case A of
        {error, Error} ->
            Error;
...
-spec be_some(gleam@option:option(GZO)) -> GZO.
be_some(A) ->
    case A of
        {some, Value} ->
            Value;
