...
-opaque subject(DTX) :: {subject, pid_(), gleam@dynamic:dynamic_()} |
    {named_subject, name(DTX)}.

-type name(DTY) :: any() | {gleam_phantom, DTY}.

-type do_not_leak() :: any().

-type selector(DTZ) :: any() | {gleam_phantom, DTZ}.
...
-spec 'receive'(subject(DUX), integer()) -> {ok, DUX} | {error, nil}.
...
-spec receive_forever(subject(DVF)) -> DVF.
...
-spec selector_receive(selector(DVJ), integer()) -> {ok, DVJ} | {error, nil}.
...
-spec selector_receive_forever(selector(DVN)) -> DVN.
...
-spec map_selector(selector(DVP), fun((DVP) -> DVR)) -> selector(DVR).
map_selector(A, B) ->
...
-spec merge_selector(selector(DVT), selector(DVT)) -> selector(DVT).
merge_selector(A, B) ->
...
-spec select_map(selector(DWE), subject(DWG), fun((DWG) -> DWE)) -> selector(DWE).
...
-spec select(selector(DWA), subject(DWA)) -> selector(DWA).
...
-spec select_record(
    ...
    any(),
    integer(),
    fun((gleam@dynamic:dynamic_()) -> DWO)
) -> selector(DWO).
...
-spec select_other(selector(DWS), fun((gleam@dynamic:dynamic_()) -> DWS)) -> selector(DWS).
...
-spec deselect(selector(DWJ), subject(any())) -> selector(DWJ).
...
-spec select_specific_monitor(selector(DXE), monitor(), fun((down()) -> DXE)) -> selector(DXE).
...
-spec select_monitors(selector(DXH), fun((down()) -> DXH)) -> selector(DXH).
...
-spec select_trapped_exits(selector(DVX), fun((exit_message()) -> DVX)) -> selector(DVX).
...
-spec deselect_specific_monitor(selector(DXK), monitor()) -> selector(DXK).
...
-spec send_after(subject(DYG), integer(), DYG) -> timer().
...
-spec perform_call(
    subject(DXN),
    fun((subject(DXP)) -> DXN),
    fun((selector(DXP)) -> {ok, DXP} | {error, nil})
) -> DXP.
...
-spec call(subject(DXU), integer(), fun((subject(DXW)) -> DXU)) -> DXW.
...
-spec call_forever(subject(DXY), fun((subject(DYA)) -> DXY)) -> DYA.
